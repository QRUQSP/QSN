...
//
function qruqsp_qsn_checkAccess(&$q, $station_id, $method) {
    //
    // Check if the station is active and the module is enabled
    //
    qruqsp_core_loadMethod($q, 'qruqsp', 'core', 'private', 'checkModuleAccess');
    $rc = qruqsp_core_checkModuleAccess($q, $station_id, array('package'=>'qruqsp', 'module'=>'qsn'));
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.qsn.1', 'msg'=>'Access denied.'));
    }
    //
    ...
    // By default fail
    //
    return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.qsn.2', 'msg'=>'Access denied'));
}
?>
